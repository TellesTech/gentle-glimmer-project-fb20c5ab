...
export default function QuickReportWizard() {
    ...
    const contextFromState = (location.state || null);
    const { role } = useAuth();
    ...
    const initialData = useMemo(() => {
        ...
        if (role === 'admin') {
            if (companies.length > 1)
                return undefined;
            if (adminSiteData) {
                const company = adminSiteData.companies;
                return {
    ...
    const handleSelectionComplete = useCallback((data) => {
    ...
    if ((companiesCount ?? 0) === 0 && !isAdminRole) {
        return (<EmptyState icon={Building2} title="Nenhuma fábrica cadastrada" description="Peça a um administrador para cadastrar uma fábrica antes de criar relatórios."/>);
    }
