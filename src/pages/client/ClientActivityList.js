...
import { CheckCircle2, Clock, Wrench, Pencil, Plus } from 'lucide-react';
...
const shiftLabel = { morning: 'Manhã', afternoon: 'Tarde', night: 'Noite' };
export default function ClientActivityList() {
    ...
    const { data: activityInfo } = useQuery({
        ...
        queryFn: async () => {
            ...
            const scoped = (urlYear && urlMonth)
                ...
                : allReports;
            const inputReports = scoped.map(r => ({
                id: r.id,
                date: r.date,
                location: r.location,
            ...
            if (group) {
                const groupSiteId = scoped.find((r) => group.reportIds.includes(r.id))?.project?.site_id ??
                    allReports.find((r) => group.reportIds.includes(r.id))?.project?.site_id ??
                    urlSiteId ??
                    null;
                return {
                    name: group.name,
                    ...
                    companyId: scoped.find((r) => group.reportIds.includes(r.id))?.project?.site?.company?.id || null,
            ...
            if (proj) {
                ...
                return { name: proj.name, reportIds: (rds || []).map(r => r.id), siteId: proj.site_id ?? null };
            }
            return null;
        },
    });
    const { data: reports = [], isLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            return (rs || [])
                ...
                .filter((r) => r.status === 'sent' || r.status === 'signed' || r.status === 'finalized')
                .map((r) => {
    ...
    const getStatusBadge = (s) => {
        if (s === 'completed')
            return <Badge className="bg-emerald-500 hover:bg-emerald-500 text-white border-transparent gap-1"><CheckCircle2 className="h-3 w-3"/>Assinado</Badge>;
        if (s === 'partial')
            return <Badge className="bg-amber-500 hover:bg-amber-500 text-white border-transparent gap-1"><Clock className="h-3 w-3"/>Parcial</Badge>;
    ...
    const activitySiteIds = useMemo(() => (activityInfo?.siteId ? [activityInfo.siteId] : []), [activityInfo?.siteId]);
    const { names: activityNames, rename: renameActivity, resetName: resetActivityName, isSaving: renamingActivity } = useActivityNames(activitySiteIds);
    ...
    return (<ClientLayout>
      <div className="space-y-5">
        ...
        <PageBackHeader onBack={() => navigate(`/client/dashboard?${searchParams.toString()}`)} icon={<Wrench className="h-5 w-5"/>} title={displayName} className="mb-0" actions={<div className="flex items-center gap-2">
              {isAdminView && (<Button type="button" variant="outline" size="sm" className="gap-1.5" onClick={() => navigate('/reports/new', {
                    state: {
              ...
              <Button type="button" variant="ghost" size="sm" className="gap-1.5" onClick={() => setRenameOpen(true)}>
        ...
        {isLoading ? (<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-8 sm:gap-10 pt-1 pb-4">
            {[...Array(6)].map((_, i) => (<Skeleton key={i} className="w-24 h-32 sm:w-32 sm:h-40 rounded-lg"/>))}
          </div>) : reports.length === 0 ? (<Card>
          ...
          </Card>) : (<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-8 sm:gap-10 pt-1 pb-4">
            {reports.map((r) => (<div key={r.id} className="flex flex-col items-center gap-3 group cursor-pointer" onClick={() => navigate(`/client/reports/${r.id}?${searchParams.toString()}`)}>
                <div className="relative w-20 h-[6.5rem] sm:w-24 sm:h-32 transition-transform duration-200 group-hover:-translate-y-1 group-hover:scale-105 group-active:scale-95">
                  ...
                  <div className="absolute inset-0 bg-white border border-border rounded-sm shadow-md overflow-hidden">
                    {/* Status stripe */}
                    <div className={cn("h-1.5 w-full", r.approverStatus === 'completed' ? "bg-emerald-500" : r.approverStatus === 'pending' ? "bg-red-500" : "bg-amber-500")}/>
                  ...
                  <div className="absolute -top-1 -right-1 z-30">
                    {r.approverStatus === 'completed' ? (<div className="bg-emerald-500 rounded-full p-0.5 shadow-sm">
                        <CheckCircle2 className="h-3 w-3 text-white"/>
                      </div>) : (<div className={cn("rounded-full p-0.5 shadow-sm", r.approverStatus === 'pending' ? "bg-red-500" : "bg-amber-500")}>
                        <Clock className="h-3 w-3 text-white"/>
                      </div>)}
                  </div>
                </div>
                <div className="text-center min-w-0 px-1 w-full">
                  <p className={cn("font-bold text-sm truncate transition-colors", r.approverStatus === 'pending' ? "text-red-600 group-hover:text-red-700" : "text-foreground group-hover:text-primary")}>
      ...
      <RenameActivityDialog target={renameOpen
            ? {
                groupKey: projectId || '',
            ...
            : null} isSaving={renamingActivity} onOpenChange={(open) => setRenameOpen(open)} onSave={async (name) => {
            if (!activityInfo?.siteId || !projectId)
                return;
