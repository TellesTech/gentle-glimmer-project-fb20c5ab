...
import { FileText, CheckCircle, Clock, PenTool, AlertTriangle, Eye, Activity, Building2, Calendar, Download, Loader2, } from 'lucide-react';
...
function buildRdoCardImage(rdoNumber, dateLabel, status) {
...
export default function ClientDashboard() {
    ...
    const { data: adminReportsData, isLoading: adminReportsLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            let effectiveSiteIds = [];
            if (adminSiteId) {
                if (!isSuper && allowedSiteIds && !allowedSiteIds.has(adminSiteId))
                    return [];
            ...
            return (data || []).map((r) => {
                ...
                return {
                    ...
                    report: {
                        ...
                        project: {
                            ...r.project,
                            site_name: r.project?.site?.name || '',
                            company_name: r.project?.site?.company?.name || '',
                        },
                    },
                };
            });
    ...
    const { data: clientReportsData, isLoading: clientReportsLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            const approverByReport = new Map();
            if (isContact) {
                ...
                (rcoa || []).forEach((r) => approverByReport.set(r.report_id, r));
            }
            else {
                ...
                (rca || []).forEach((r) => approverByReport.set(r.report_id, r));
            }
            ...
            const { data: siteRows } = await supabase.rpc('portal_user_site_ids', {
                _user_id: user?.id,
            });
            const siteIds = (siteRows || [])
                .map((s) => (typeof s === 'string' ? s : s?.portal_user_site_ids))
                .filter(Boolean);
            if (!siteIds.length)
                return [];
    ...
    const { data: hiddenMonths } = useQuery({
        ...
        queryFn: async () => {
            ...
            else if (hiddenScopeCompanyId)
                q = q.eq('company_id', hiddenScopeCompanyId);
            const { data } = await q;
            return (data || []);
        },
        enabled: !!hiddenScopeCompanyId || !!adminSiteId,
    });
    const hiddenMonthKeys = useMemo(() => new Set((hiddenMonths || []).map(h => `${h.year}-${h.month}`)), [hiddenMonths]);
    ...
    const visibleReports = useMemo(() => {
        ...
        if (isSuperAdmin || hiddenMonthKeys.size === 0)
            return all;
        return all.filter(r => {
            if (!r.report?.date)
                return true;
    ...
    const { data: weesSignedIds } = useQuery({
        ...
        queryFn: async () => {
            ...
            return new Set((data || []).filter((s) => !!s.signature_data).map((s) => s.report_id));
        },
    ...
    const metrics = useMemo(() => {
        ...
        all.forEach(r => { if (r.report?.project?.id)
            projectSet.add(r.report.project.id); });
        // Score (0-100)
        let score = 100;
        score -= overdue.length * 5;
        if (avgSignatureDays > 3)
            score -= Math.round((avgSignatureDays - 3) * 2);
    ...
    const toggleHiddenMonth = async (e, month, hidden) => {
        e.stopPropagation();
        if (!canToggleHiddenMonth)
            return;
        try {
            if (hidden) {
    ...
    const reportSiteIds = useMemo(() => {
        ...
        visibleReports.forEach(r => {
            const sid = r.report?.project?.site_id;
            if (sid)
                s.add(sid);
        });
        return Array.from(s);
    }, [visibleReports]);
    const { names: activityNames, rename: renameActivity, resetName: resetActivityName, isSaving: renamingActivity } = useActivityNames(reportSiteIds);
    ...
    const monthFolders = useMemo(() => {
        ...
        return Array.from(monthMap.values())
            ...
            const inputReports = reports.map(r => ({
                ...
                maintenance_order_number: r.report.maintenance_order_number,
                maintenance_order_title: r.report.maintenance_order_title,
            ...
            const activities = groups.map(group => {
                ...
                const partial = groupReports.filter(r => r.status !== 'approved' && weesSignedIds?.has(r.report_id)).length;
                return {
                    id: group.id,
                    name: group.name,
                    siteId: groupReports.find(r => r.report?.project?.site_id)?.report?.project?.site_id ?? null,
            ...
            const partial = reports.filter(r => r.status !== 'approved' && weesSignedIds?.has(r.report_id)).length;
            return {
                id: `${year}-${month}`,
                year,
                month,
    ...
    const visibleMonthFolders = useMemo(() => (isSuperAdmin ? monthFolders : monthFolders.filter(m => !hiddenMonthKeys.has(`${m.year}-${m.month}`))), [monthFolders, hiddenMonthKeys, isSuperAdmin]);
    ...
    const [downloadingMonthId, setDownloadingMonthId] = useState(null);
    const [downloadProgress, setDownloadProgress] = useState(null);
    const handleDownloadMonth = async (e, month) => {
        e.stopPropagation();
        if (downloadingMonthId)
            return;
        const items = month.activities.flatMap((a) => a.reports.map((r) => ({ ...r, activityName: a.name })));
        if (items.length === 0) {
    ...
    const { data: coverPhotosMap } = useQuery({
        ...
        queryFn: async () => {
            ...
            const m = new Map();
            (data || []).forEach((row) => {
                if (!m.has(row.report_id) && row.photo_url)
                    m.set(row.report_id, row.photo_url);
            });
            return m;
        },
    ...
    const handleApproveSignature = async (reportApproverId, reportId) => {
    ...
    return (<ClientLayout>
      <div className="space-y-5 sm:space-y-6 min-w-0">
        {/* Collaborator Banner */}
        {isCollaboratorView && (<div className="rounded-xl border border-primary/30 bg-primary/5 p-4 sm:p-5">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2">
                {effectiveSignatureData ? (<Badge variant="outline" className="border-[hsl(var(--success))] text-[hsl(var(--success))]">
                    <CheckCircle className="h-3 w-3 mr-1"/> Assinatura ativa
                  </Badge>) : (<Button size="sm" variant="outline" onClick={() => navigate(`/client/profile?${searchParams.toString()}`)}>
                    <PenTool className="h-4 w-4 mr-1"/> Configurar assinatura
                  </Button>)}
                {pendingList.length > 0 && effectiveSignatureData && (<Badge variant="default">{pendingList.length} pendência(s)</Badge>)}
              </div>
            </div>
          </div>)}

        {/* Page identifier — Portal {Empresa} */}
        {portalContextName && (<div className="flex items-center gap-3 rounded-xl border border-primary/10 bg-primary/5 px-4 py-3 shadow-sm">
        ...
        {/* Signature Alert */}
        {portalSettings.show_supersign_alert !== false && !effectiveSignatureData && (<Alert variant="destructive">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex items-center gap-2 px-3 py-1.5 bg-card rounded-full border border-border shadow-sm">
            ...
            {reportsLoading ? <Skeleton className="h-4 w-6"/> : (<span className="text-sm font-bold ml-1">{metrics.total}</span>)}
          </div>
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 bg-[hsl(var(--success))/5] rounded-full border border-[hsl(var(--success))/20] shadow-sm">
            ...
            {reportsLoading ? <Skeleton className="h-4 w-6"/> : (<span className="text-sm font-bold ml-1 text-[hsl(var(--success))]">{metrics.signed}</span>)}
          </div>
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 bg-primary/5 rounded-full border border-primary/20 shadow-sm">
            ...
            {reportsLoading ? <Skeleton className="h-4 w-6"/> : (<span className="text-sm font-bold ml-1 text-primary">{metrics.pending}</span>)}
          </div>
        </div>
        ...
        {visibleMonthFolders.length > 0 && (<div className="space-y-6">
            {!selectedMonthId ? (<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-8 sm:gap-10 py-4">
                {visibleMonthFolders.map((month) => {
                    const isHidden = hiddenMonthKeys.has(`${month.year}-${month.month}`);
                    return (<div key={month.id} className={cn('flex flex-col items-center gap-3 group cursor-pointer', isHidden && 'opacity-50')} onClick={() => setSelectedMonthId(month.id)}>
                    <div className="relative w-24 h-20 sm:w-32 sm:h-24 transition-transform duration-200 group-hover:scale-105 group-active:scale-95">
                      ...
                      <button type="button" title="Baixar RDOs do mês (ZIP)" onClick={(e) => handleDownloadMonth(e, month)} disabled={downloadingMonthId === month.id} className="absolute -top-2 -right-2 z-30 rounded-full bg-background border shadow-sm p-1.5 text-muted-foreground hover:text-primary hover:border-primary transition-colors opacity-100 md:opacity-0 md:group-hover:opacity-100 disabled:opacity-100">
                        {downloadingMonthId === month.id ? (<Loader2 className="h-3.5 w-3.5 animate-spin"/>) : (<Download className="h-3.5 w-3.5"/>)}
                      </button>

                      {/* Ocultar/reexibir pasta do mês (somente super admin) */}
                      {canToggleHiddenMonth && (<button type="button" title={isHidden ? 'Reexibir pasta para o cliente' : 'Ocultar pasta do cliente'} onClick={(e) => toggleHiddenMonth(e, month, isHidden)} className="absolute -top-2 -left-2 z-30 rounded-full bg-background border shadow-sm p-1.5 text-muted-foreground hover:text-primary hover:border-primary transition-colors opacity-100 md:opacity-0 md:group-hover:opacity-100">
                    ...
                    <div className="text-center min-w-0 px-1">
                      ...
                      {isHidden && (<Badge variant="secondary" className="mt-1 h-4 px-1.5 text-[10px]">Oculto</Badge>)}
                      {downloadingMonthId === month.id && downloadProgress && (<p className="text-[10px] text-primary font-semibold mt-0.5">
                          {downloadProgress.done}/{downloadProgress.total}
                        </p>)}
                    </div>
                  </div>);
                })}
              </div>) : (<div className="space-y-1 animate-in fade-in slide-in-from-left-4 duration-300">
                <PageBackHeader onBack={() => setSelectedMonthId(null)} icon={<Calendar className="h-5 w-5"/>} iconClassName="bg-yellow-100 text-yellow-700" title={`${visibleMonthFolders.find(m => m.id === selectedMonthId)?.monthName ?? ''} ${visibleMonthFolders.find(m => m.id === selectedMonthId)?.year ?? ''}`} className="mb-0"/>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-8 sm:gap-10 pt-1 pb-4">
                  {visibleMonthFolders.find(m => m.id === selectedMonthId)?.activities.map((a) => {
                    ...
                    return (<div key={a.id} className="flex flex-col items-center gap-3 group cursor-pointer self-start" onClick={() => {
                        ...
                        <div className="relative w-24 h-20 sm:w-32 sm:h-24 transition-transform duration-200 group-hover:scale-105 group-active:scale-95">
                          ...
                          <div className="absolute -top-2 -right-2 z-30 flex gap-1 transition-opacity opacity-100 md:opacity-0 md:group-hover:opacity-100">
                            {isAdminView && (<button type="button" title="Novo Relatório" onClick={(e) => {
                                e.stopPropagation();
                                const folder = visibleMonthFolders.find(m => m.id === selectedMonthId);
                                navigate('/reports/new', {
                                    state: {
                                        companyId: adminCompanyId,
                                        companyName: visibleReports.find(r => r.report?.project?.id === a.id)?.report?.project?.company?.name || clientProfile?.company_id,
                                        siteId: a.siteId || adminSiteId,
                                        siteName: visibleReports.find(r => r.report?.project?.id === a.id)?.report?.project?.site?.name,
                          ...
                          {/* Folder Rear Part */}
                          <div className={cn("absolute inset-x-0 bottom-0 top-3 rounded-lg shadow-sm", status === 'pending' ? "bg-red-200" : "bg-[#e5b52a]")}/>
                          {/* Folder Tab */}
                          <div className={cn("absolute top-0 left-0 w-10 h-6 rounded-t-lg", status === 'pending' ? "bg-red-200" : "bg-[#e5b52a]")}/>
                          {/* Folder Front Part */}
                          <div className={cn("absolute inset-x-0 bottom-0 top-5 rounded-lg shadow-md transition-transform group-hover:-rotate-1 origin-bottom-left", status === 'pending' ? "bg-red-500" : "bg-[#f4c430]")}/>
                          
                          {/* Status Icon Overlay */}
                          <div className="absolute inset-0 flex items-center justify-center pt-4">
                            {status === 'completed' ? (<CheckCircle className="h-6 w-6 text-green-800/40"/>) : (<Activity className={cn("h-6 w-6", status === 'pending' ? "text-white/40" : "text-yellow-900/30")}/>)}
                          </div>
                        </div>
                        <div className="text-center min-w-0 px-1 w-full">
                          <p title={a.name} className={cn("font-bold text-sm break-words leading-snug transition-colors", status === 'pending' ? "text-red-600 group-hover:text-red-700" : "text-foreground group-hover:text-primary")}>
                            {a.name}
                          </p>
      ...
      <RenameActivityDialog target={renameTarget} isSaving={renamingActivity} onOpenChange={(open) => { if (!open)
        setRenameTarget(null); }} onSave={async (name) => {
