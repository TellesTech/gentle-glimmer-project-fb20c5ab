...
import { QuickReportFormContent } from '@/components/reports/QuickReportFormContent';
...
export default function SimplifiedReportForm() {
    ...
    const hasSelectionState = navigationState?.projectId ? {
    ...
    const [autoGenData, setAutoGenData] = useState(null);
    ...
    const selection = isEditMode && existingReport
        ...
        : hasSelectionState || (projectData ? {
            ...
            companyName: projectData.companies?.name || '',
            siteId: projectData.site_id,
            siteName: projectData.sites?.name || '',
    ...
    const editModeInitialData = existingReport ? {
        ...
        activities: (existingReport.activities || []).map((a) => ({
        ...
        deviations: (existingReport.deviations || []).map((d) => ({
            type: d.type || 'other',
            description: d.description || '',
            impact: d.impact || 'low',
        })),
        photos: (existingReport.photos || []).map((p) => p.url),
        ...
        activitySteps: (existingReport.activity_steps || []).map((s) => ({
            id: s.id,
        ...
        additionalDelays: (existingReport.additional_delays || []).map((d) => {
    ...
    const [savedTabReports, setSavedTabReports] = useState({});
    const createReportMutation = useMutation({
        mutationFn: async ({ data, status }) => {
            const getCompanyIdFromProject = async (projId) => {
            ...
            try {
                const { data: lastRdo } = await supabase
                    ...
                    .limit(1)
                    .maybeSingle();
                nextRdoNumber = (lastRdo?.rdo_number ?? 0) + 1;
            }
            catch {
                nextRdoNumber = null;
            }
            ...
            if (data.activitySteps && data.activitySteps.length > 0) {
                ...
                if (stepsError)
                    throw new Error('Erro ao salvar etapas: ' + stepsError.message);
            }
            // Insert activities
            if (data.activities.length > 0) {
                ...
                if (activitiesData.length > 0) {
                    ...
                    if (actError)
                        throw new Error('Erro ao salvar atividades: ' + actError.message);
                }
            }
            // Insert attendance
            if (data.attendance.length > 0) {
                ...
                if (attError)
                    throw new Error('Erro ao salvar efetivo: ' + attError.message);
            }
            // Insert deviations
            if (data.hasDeviations && data.deviations.length > 0) {
                ...
                if (deviationsData.length > 0) {
                    ...
                    if (devError)
                        throw new Error('Erro ao salvar desvios: ' + devError.message);
                }
            }
            // Insert photos
            if (data.photos.length > 0) {
                ...
                if (photoError)
                    throw new Error('Erro ao salvar fotos: ' + photoError.message);
            }
            ...
            if (data.additionalDelays && data.additionalDelays.length > 0) {
                ...
                if (delaysData.length > 0) {
                    ...
                    if (delaysError)
                        throw new Error('Erro ao salvar atrasos adicionais: ' + delaysError.message);
                }
            }
            return report;
        },
        onSuccess: async (report, { data, status }) => {
            ...
            let companyId = selection?.companyId;
            ...
            if (selection?.siteId)
                params.set('site', selection.siteId);
    ...
    const updateReportMutation = useMutation({
        mutationFn: async ({ data, status }) => {
            ...
            if (deleteError?.error)
                throw new Error('Erro ao limpar dados anteriores: ' + deleteError.error.message);
            ...
            if (data.additionalDelays) {
                const companyId = selection.companyId || existingReport?.project?.company_id;
    ...
    return (<>
      ...
      {showTabs && (<ReportTabs tabs={tabsHook.tabs} activeTabId={tabsHook.activeTabId} onAddTab={tabsHook.addTab} onAddTabFromExisting={() => tabsHook.addTabFromExisting(tabsHook.activeTabId)} onRemoveTab={tabsHook.removeTab} onSelectTab={tabsHook.setActiveTab} canAddTab={tabsHook.canAddTab}/>)}
      
      <div className="rounded-lg">
        <QuickReportFormContent key={showTabs ? tabsHook.activeTabId : reportId} selection={selection} onBack={handleBack} onSubmit={handleSubmit} isSubmitting={isSubmitting} initialData={currentFormData} isEditMode={isEditMode} tabId={showTabs ? tabsHook.activeTabId : undefined} onFormDataChange={showTabs ? handleFormDataChange : undefined}/>
      </div>

      <ConfirmDialog open={showLeaveDialog} onOpenChange={setShowLeaveDialog} title="Sair sem salvar?" description="Você tem alterações não salvas em uma ou mais abas. Deseja descartar as alterações e sair?" confirmText="Sair" cancelText="Continuar editando" variant="destructive" onConfirm={confirmLeave}/>

      {autoGenData && (<AutoServiceReportDialog open={showAutoGenDialog} onOpenChange={setShowAutoGenDialog} projectId={autoGenData.projectId} siteId={autoGenData.siteId} reportId={autoGenData.reportId}/>)}
    </>);
}
