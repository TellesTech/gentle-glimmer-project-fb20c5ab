...
export default function ReportForm() {
    ...
    const { data: allProfiles = [] } = useQuery({
        ...
        `)
            ...
            return (data || []).map(p => ({
                ...
                siteIds: p.site_responsibles?.map((sr) => sr.site_id) || []
            }));
        },
    });
    ...
    useEffect(() => {
        if (existingReport && isEditing && !dataLoaded) {
            setFormData({
                ...
                clientName: existingReport.client_name || '',
                clientCompany: existingReport.client_company || '',
                ...
                blockRevalidationTime: '',
                blockageStatus: '',
                activities: existingReport.activities?.map((a, idx) => ({
                    id: a.id,
                ...
                attendance: existingReport.attendance?.map((att) => ({
                    id: att.id,
                    reportId: existingReport.id,
    ...
    useEffect(() => {
        if (copyFromReport && !isEditing && !dataLoaded) {
            setFormData(prev => ({
                ...
                attendance: copyFromReport.attendance?.map((att, idx) => ({
    ...
    const saveReportToDatabase = async (status) => {
        if (!user?.id) {
        ...
        if (isEditing && id) {
            ...
            const existingActivityIds = existingReport?.activities?.map((a) => a.id) || [];
            ...
            const existingDeviationIds = existingReport?.deviations?.map((d) => d.id) || [];
            const currentDeviationIds = formData.deviations.filter(d => d.id).map(d => d.id);
            const deviationsToDelete = existingDeviationIds.filter((id) => !currentDeviationIds.includes(id));
            ...
            const existingAttendanceIds = existingReport?.attendance?.map((a) => a.id) || [];
            const currentAttendanceIds = formData.attendance.filter(a => a.id).map(a => a.id);
            const attendanceToDelete = existingAttendanceIds.filter((id) => !currentAttendanceIds.includes(id));
            ...
            const existingPhotoUrls = existingReport?.photos?.map((p) => p.url) || [];
            const currentPhotoUrls = formData.photos;
            const photosToDelete = existingReport?.photos?.filter((p) => !currentPhotoUrls.includes(p.url)).map((p) => p.id) || [];
            ...
            if (newPhotos.length > 0) {
                await supabase.from('report_photos').insert(newPhotos.map(url => ({ report_id: reportId, url, description: null })));
            }
            return { id: reportId };
        }
        else {
        ...
        switch (currentStep) {
            case 1:
                ...
                const siteFilteredProfiles = allProfiles.filter(p => {
                    if (!currentSiteIdForBasic)
                        return true;
                    const pSiteIds = p.siteIds || [];
                    if (pSiteIds.length > 0) {
                        return pSiteIds.includes(currentSiteIdForBasic);
                    }
                    return true;
                });
                return (<StepBasicInfo data={formData} onChange={updateFormData} teams={teamsForStep} projects={projectsForStep} eligibleSupervisors={eligibleSupervisors} allProfiles={siteFilteredProfiles}/>);
            case 2:
            ...
            case 6:
                ...
                return (<StepAttendance data={formData} onChange={updateFormData} teamMembers={teamMembersForStep} allProfiles={allProfiles.filter(p => {
                        if (!currentSiteId)
                            return false;
                        const pSiteIds = p.siteIds || [];
                        return pSiteIds.includes(currentSiteId);
                    })} defaultArrivalTime={formData.startTime || '07:00'} defaultDepartureTime={formData.endTime || '17:00'}/>);
            case 7:
                return (<StepPhotosReview data={formData} onChange={updateFormData} projects={projectsForStep} teams={teamsForStep}/>);
            default:
                return null;
        }
    };
    const handleAIParsedData = (parsedData) => {
        setFormData(prev => ({
            ...prev,
            ...parsedData,
        }));
    };
    return (<div className="max-w-3xl mx-auto pb-24 md:pb-8">
      {/* Header */}
      <div className="flex items-center gap-4 mb-6">
        ...
        {!isEditing && (<ParseReportModal onDataParsed={handleAIParsedData} teamMembers={teamMembers} allProfiles={allProfiles}/>)}
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 md:relative md:mt-6 p-4 bg-background border-t md:border-0 md:p-0 flex items-center justify-between gap-3 z-40">
        ...
        <div className="flex gap-2">
          ...
          {currentStep === steps.length ? (<Button onClick={handleSubmit} disabled={isSaving} className="gap-2">
