...
import { Folder, FileText, ChevronRight, Building2, MapPin, Calendar, Download, Loader2, HardHat, FolderKanban, MoreVertical, Pencil, Trash2, Plus } from 'lucide-react';
import { cn } from '@/lib/utils';
import { useActivityNames } from '@/hooks/useActivityNames';
import { RenameActivityDialog } from '@/components/reports/RenameActivityDialog';
...
import { exportReportsBatch, uploadBatchExportToCloud, } from '@/lib/generateBatchReportsPdf';
import { triggerDownloadFromBlob } from '@/lib/downloadUtils';
import { BatchDownloadOptionsDialog } from './BatchDownloadOptionsDialog';
import { sanitizeOmNumber, normalizeOmKeyNumber, normalizeOmTitle, omTitleTokens, tokenSimilarity, TITLE_MERGE_THRESHOLD, } from '@/lib/rdoActivityGroups';
...
export function DocumentCabinet({ onBreadcrumbChange, onContextChange }) {
    ...
    const setOpenCompanyId = useCallback((id) => {
    ...
    const setOpenSiteId = useCallback((id) => {
        setSearchParams(prev => {
            const next = new URLSearchParams(prev);
            if (id) {
                next.set('site', id);
    ...
    const setOpenYear = useCallback((year) => {
        setSearchParams(prev => {
            const next = new URLSearchParams(prev);
            if (year !== null) {
    ...
    const setOpenMonth = useCallback((month) => {
        setSearchParams(prev => {
            const next = new URLSearchParams(prev);
            if (month !== null) {
    ...
    const setOpenProjectId = useCallback((id) => {
        setSearchParams(prev => {
            const next = new URLSearchParams(prev);
            if (id) {
                next.set('project', id);
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            if (deletingItem.type === 'report' || deletingItem.type === 'reportGroup') {
                const ids = deletingItem.type === 'report'
                    ? [deletingItem.id]
            ...
            if (count === 0) {
                ...
                if (deletingItem.type === 'company' && deletingItem.id === openCompanyId)
                    setOpenCompanyId(null);
                if (deletingItem.type === 'site' && deletingItem.id === openSiteId)
                    setOpenSiteId(null);
                if (deletingItem.type === 'project' && deletingItem.id === openProjectId)
    ...
    const mergeReportsMutation = useMutation({
        mutationFn: async ({ sourceReportId, targetFolder }) => {
    ...
    const CardActions = ({ id, type, name, onEdit, reportIds }) => {
        if (!isSuperAdmin)
            return null;
        return (<DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="ghost" size="icon" className="h-7 w-7 pointer-events-auto" onClick={(e) => { e.preventDefault(); e.stopPropagation(); }}>
    ...
    const openDownloadOptions = (e, reportIds, folderName, folderId) => {
        e.stopPropagation();
    ...
    const handleDownloadWithOptions = async (options) => {
        ...
        try {
            const { blob } = await exportReportsBatch(reportIds, 'zip', (progress) => setExportProgress(progress), pdfOptions);
    ...
    const DownloadButton = ({ reportIds, folderName, folderId, size = 'default' }) => {
        const isThisExporting = isExporting && exportingId === folderId;
        const iconSize = size === 'sm' ? 'h-3.5 w-3.5' : 'h-4 w-4';
        return (<Button type="button" variant="ghost" size={size === 'sm' ? 'icon' : 'sm'} className={size === 'sm' ? 'h-7 w-7 pointer-events-auto' : 'gap-2 pointer-events-auto'} onPointerDown={(e) => openDownloadOptions(e, reportIds, folderName, folderId)} onClick={(e) => openDownloadOptions(e, reportIds, folderName, folderId)} disabled={isExporting} title={`Baixar ${reportIds.length} relatório(s) como ZIP`}>
        {isThisExporting ? (<>
            <Loader2 className={`${iconSize} animate-spin`}/>
            {size !== 'sm' && exportProgress && (<span className="text-xs">
    ...
    const cabinetSiteIds = useMemo(() => (allSites || []).map((s) => s.id).filter(Boolean), [allSites]);
    const { namesBySite: activityNamesBySite, rename: renameActivity, resetName: resetActivityName, isSaving: renamingActivity, } = useActivityNames(cabinetSiteIds);
    ...
    const reports = useMemo(() => {
        ...
        ownReports.forEach(r => { if (!byId.has(r.id))
            byId.set(r.id, r); });
    ...
    const companyFolders = useMemo(() => {
        ...
        const sitesByIdLocal = new Map(allSites.map((s) => [s.id, s]));
        const companiesByIdLocal = new Map(allCompanies.map((c) => [c.id, c]));
        ...
        reports.forEach(report => {
            const rawProjectId = report.project?.id || report.project_id || null;
            const fallbackProject = rawProjectId ? projectsById.get(rawProjectId) : null;
            const project = report.project || fallbackProject;
            if (!project)
                return;
            const fallbackSite = sitesByIdLocal.get(report.project?.site?.id || project.site_id);
            const site = report.project?.site || fallbackSite;
            if (!site)
                return;
            const company = report.project?.site?.company || companiesByIdLocal.get(site.company_id);
            if (!company)
                return;
            ...
            if (rawOmNum && !projectFolder.omNumbers.includes(rawOmNum))
                projectFolder.omNumbers.push(rawOmNum);
            if (omTitle) {
                if (!projectFolder.omTitles.includes(omTitle))
                    projectFolder.omTitles.push(omTitle);
            ...
            projectFolder.progress = Math.min(Math.round((projectFolder.progress + (report.daily_progress || 0)) * 10) / 10, 100);
                ...
                site.years.forEach(year => {
                    ...
                    year.months.forEach(month => {
                        ...
                        month.projects.forEach(pf => {
                            ...
                            if (!target.lastDate || (pf.lastDate && pf.lastDate > target.lastDate))
                                target.lastDate = pf.lastDate;
                            pf.omNumbers.forEach(n => { if (!target.omNumbers.includes(n))
                                target.omNumbers.push(n); });
                            pf.omTitles.forEach(t => { if (!target.omTitles.includes(t))
                                target.omTitles.push(t); });
                            pf.sourceProjects.forEach(sp => {
                                if (!target.sourceProjects.some(s => s.id === sp.id))
                                    target.sourceProjects.push(sp);
                            });
    ...
    const availableYears = useMemo(() => {
        ...
        allProjects.forEach((p) => {
            if (p.created_at)
                years.add(getYear(parseISO(p.created_at)));
        });
    ...
    const dialogs = (<>
      <RenameActivityDialog target={renameTarget} onOpenChange={(open) => { if (!open)
        setRenameTarget(null); }} isSaving={renamingActivity} onSave={async (name) => {
            if (!renameTarget?.siteId)
                return;
      ...
      <BatchDownloadOptionsDialog open={downloadDialogOpen} onOpenChange={setDownloadDialogOpen} onConfirm={handleDownloadWithOptions} reportCount={pendingDownload?.reportIds.length || 0} folderName={pendingDownload?.folderName || ''}/>
      <ConfirmDialog open={!!deletingItem} onOpenChange={(open) => !open && setDeletingItem(null)} title={`Excluir ${deletingItem?.type === 'company'
            ? 'empresa'
            : deletingItem?.type === 'site'
                ? 'unidade'
                : deletingItem?.type === 'project'
                    ...
                    : deletingItem?.type === 'reportGroup'
                        ? 'RDOs da pasta'
                        : 'relatório'}`} description={deletingItem?.type === 'report'
            ...
            : deletingItem?.type === 'reportGroup'
                ...
                : `ATENÇÃO: excluir "${deletingItem?.name}" também remove ${deleteImpactCount} RDO(s) vinculado(s) e todos os seus dados. Esta ação não pode ser desfeita.`} confirmText="Excluir" variant="destructive" onConfirm={handleDelete} isLoading={isDeleting}/>
    ...
    if (isLoading) {
        return (<div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
        {[...Array(4)].map((_, i) => (<div key={i} className="h-40 bg-muted/50 rounded-xl animate-pulse"/>))}
      </div>);
    }
    ...
    if (selectedCompany && selectedSiteFolder && selectedYearFolder && selectedMonthFolder && selectedProjectFolder) {
        ...
        const seqLabel = (report) => {
            const real = Number(report.rdo_number);
            if (Number.isFinite(real) && real > 0)
                return String(real).padStart(3, '0');
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between pb-2 border-b">
            ...
            <div className="flex items-center gap-2">
              <Button type="button" variant="outline" size="sm" className="gap-1.5" onClick={() => navigate('/reports/new', {
                state: {
              ...
              <Button type="button" variant="outline" size="sm" className="gap-1.5" onClick={() => setRenameTarget({
              ...
              <DownloadButton reportIds={selectedProjectFolder.reports.map(r => r.id)} folderName={`${selectedCompany.name}_${selectedSiteFolder.name}_${selectedYearFolder.year}_${selectedMonthFolder.monthName}_${selectedProjectFolder.omNumber ? `OM-${selectedProjectFolder.omNumber}` : selectedProjectFolder.name}`} folderId={`om-${selectedProjectFolder.id}-${openMonth}-${openYear}`}/>
            </div>
          </div>
            ...
            <div className="flex flex-col lg:flex-row gap-6">
              ...
              <div className="flex-1">
                ...
                <Droppable droppableId="reports-list">
                  {(provided) => (<div {...provided.droppableProps} ref={provided.innerRef} className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {selectedProjectFolder.reports.map((report, index) => (<Draggable key={report.id} draggableId={report.id} index={index}>
                          {(provided, snapshot) => (<div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps} onClick={() => navigate(`/reports/${report.id}`)} className={cn("relative rounded-xl border bg-card p-4 hover:bg-muted/60 transition-colors cursor-pointer shadow-sm group", snapshot.isDragging && "shadow-xl border-primary/50 ring-2 ring-primary/20 scale-105 z-50 bg-accent")}>
                              {/* Actions */}
                              {isSuperAdmin && (<div className="absolute top-2 right-2 z-10">
                                  <CardActions id={report.id} type="report" name={`RDO Nº ${seqLabel(report)}`} onEdit={() => navigate(`/reports/${report.id}/edit`)}/>
                                </div>)}
                              ...
                              <div className="flex items-center justify-between gap-2 mb-2">
                                <div className="flex items-center gap-2 min-w-0">
                                  ...
                                  <span className="text-sm font-semibold text-foreground truncate">
                                    ...
                                    {report.maintenance_order_number && (<span className="ml-1.5 text-xs font-normal text-muted-foreground">
                              ...
                              <div className="flex items-center gap-2 mb-2.5">
                                ...
                                {report.location && (<span className="flex items-center gap-1 text-[10px] text-muted-foreground truncate">
                              ...
                              <div className="flex items-center gap-1.5 flex-wrap">
                                ...
                                {report.source === 'whatsapp_ai' && (<Badge variant="secondary" className="text-[10px] px-2 py-0.5 gap-1 bg-green-500/10 text-green-600 border border-green-500/20" title="Registro criado automaticamente pela IA a partir de mensagem no WhatsApp">
              ...
              <div className="w-full lg:w-72 shrink-0">
                ...
                .map((otherProject) => (<Droppable key={otherProject.id} droppableId={otherProject.id}>
                        {(provided, snapshot) => (<div {...provided.droppableProps} ref={provided.innerRef} className={cn("rounded-lg border p-3 transition-all duration-200", snapshot.isDraggingOver
                        ? "bg-primary/5 border-primary shadow-inner scale-102 ring-2 ring-primary/20"
                        : "bg-card hover:bg-muted/50")}>
                            <div className="flex items-center gap-3">
                              <div className={cn("p-2 rounded-md transition-colors", snapshot.isDraggingOver ? "bg-primary/20" : "bg-muted")}>
                                <HardHat className={cn("h-4 w-4", snapshot.isDraggingOver ? "text-primary" : "text-muted-foreground")}/>
                              </div>
                              <div className="min-w-0 flex-1">
                                <p className={cn("text-xs font-semibold truncate", snapshot.isDraggingOver ? "text-primary" : "text-card-foreground")}>
                                  {otherProject.name}
                                </p>
                  ...
                  {selectedMonthFolder.projects.filter(p => p.id !== selectedProjectFolder.id).length === 0 && (<div className="text-center py-6 px-4 rounded-lg border-2 border-dashed border-muted bg-muted/20">
    ...
    if (selectedCompany && selectedSiteFolder && selectedYearFolder && selectedMonthFolder) {
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between pb-2 border-b">
            ...
            <DownloadButton reportIds={selectedMonthFolder.reports.map(r => r.id)} folderName={`${selectedCompany.name}_${selectedSiteFolder.name}_${selectedYearFolder.year}_${selectedMonthFolder.monthName}`} folderId={`month-${selectedCompany.id}-${openSiteId}-${openYear}-${openMonth}`}/>
          </div>

          {/* Project folders list */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {selectedMonthFolder.projects.map((projectFolder) => (<div key={projectFolder.id} className="relative rounded-xl border bg-card p-3.5 hover:bg-muted/60 transition-colors cursor-pointer shadow-sm group" onClick={() => setOpenProjectId(projectFolder.id)}>
                {/* Actions */}
                <div className="absolute top-2 right-2 z-10 flex items-center gap-1">
                  <button type="button" title="Renomear pasta" onClick={(e) => {
                    e.stopPropagation();
                    setRenameTarget({
                  ...
                  {isSuperAdmin && (<CardActions id={projectFolder.sourceProjects[0]?.id || projectFolder.id} type="reportGroup" name={projectFolder.name} reportIds={projectFolder.reports.map((r) => r.id)} onEdit={() => {
                        const ids = projectFolder.sourceProjects.map((sp) => sp.id).filter(Boolean);
                        const primary = ids[0] || '';
                        const extras = ids.slice(1);
                        navigate(`/projects/${primary}${extras.length ? `?projects=${extras.join(',')}` : ''}`);
                    }}/>)}
                </div>
                ...
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <div className="flex-1 min-w-0">
                    {projectFolder.omNumber ? (<span title={`OM ${projectFolder.omNumber}`} className="inline-block mb-0.5 px-1.5 py-0.5 rounded-md bg-primary/10 text-primary text-[10px] font-bold tracking-wide">
                        OM {projectFolder.omNumber}
                      </span>) : (<span className="inline-block mb-0.5 px-1.5 py-0.5 rounded-md bg-muted text-muted-foreground text-[10px] font-bold tracking-wide">
                        SEM Nº DE OM
                      </span>)}
                ...
                <div className="flex items-center gap-2">
                  <div className="flex-1 h-2 bg-border rounded-full overflow-hidden">
                    <div className="h-full bg-foreground rounded-full transition-all" style={{ width: `${projectFolder.progress}%` }}/>
                  </div>
                  <span className="text-xs font-bold text-foreground w-8 text-right">{Math.round(projectFolder.progress)}%</span>
                  <span className={cn("px-2 py-0.5 rounded-full text-[10px] font-medium border shrink-0", getStatusColor(projectFolder.status))}>
    ...
    if (selectedCompany && selectedSiteFolder && selectedYearFolder) {
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between pb-2 border-b">
            ...
            <DownloadButton reportIds={selectedYearFolder.reports.map(r => r.id)} folderName={`${selectedCompany.name}_${selectedSiteFolder.name}_${selectedYearFolder.year}`} folderId={`year-${selectedCompany.id}-${openSiteId}-${selectedYearFolder.year}`}/>
          </div>

          {/* Month folders grid */}
          <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
            {selectedYearFolder.months.map((monthFolder) => (<FolderCard key={monthFolder.month} onClick={() => setOpenMonth(monthFolder.month)} icon={<div className="w-[4.2rem] h-[3.6rem] rounded-lg flex items-center justify-center" style={{ backgroundColor: '#ffffff' }}>
                    <Calendar className="h-8 w-8 text-muted-foreground"/>
                  </div>} title={monthFolder.monthName} stats={[
                ...
                ]} topRightActions={<DownloadButton reportIds={monthFolder.reports.map(r => r.id)} folderName={`${selectedCompany.name}_${selectedSiteFolder.name}_${selectedYearFolder.year}_${monthFolder.monthName}`} folderId={`month-${selectedCompany.id}-${openSiteId}-${selectedYearFolder.year}-${monthFolder.month}`} size="sm"/>}/>))}
          </div>
        </div>
        {dialogs}
      </>);
    ...
    if (selectedCompany && selectedSiteFolder) {
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between pb-2 border-b">
            ...
            <DownloadButton reportIds={selectedSiteFolder.reports.map(r => r.id)} folderName={`${selectedCompany.name}_${selectedSiteFolder.name}`} folderId={`site-${selectedSiteFolder.id}`}/>
          </div>

          {/* Year folders grid */}
           <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
            {selectedSiteFolder.years.map((yearFolder) => (<FolderCard key={yearFolder.year} onClick={() => setOpenYear(yearFolder.year)} icon={<div className="w-[4.2rem] h-[3.6rem] rounded-lg flex items-center justify-center" style={{ backgroundColor: '#ffffff' }}>
                    <Calendar className="h-8 w-8 text-muted-foreground"/>
                  </div>} title={String(yearFolder.year)} stats={[
                ...
                ]} topRightActions={<DownloadButton reportIds={yearFolder.reports.map(r => r.id)} folderName={`${selectedCompany.name}_${selectedSiteFolder.name}_${yearFolder.year}`} folderId={`year-${selectedCompany.id}-${openSiteId}-${yearFolder.year}`} size="sm"/>}/>))}
          </div>
        </div>
        {dialogs}
      </>);
    ...
    if (selectedCompany) {
        ...
        <div className="space-y-4">
          {/* Site folders grid */}
          <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
            {selectedCompany.sites.map((siteFolder) => (<FolderCard key={siteFolder.id} onClick={() => setOpenSiteId(siteFolder.id)} icon={<div className="w-20 h-[4.2rem] rounded-lg overflow-hidden flex items-center justify-center" style={{ backgroundColor: '#ffffff' }}>
                    {(siteFolder.photo_url || siteFolder.company_logo_url) ? (<img src={siteFolder.photo_url || siteFolder.company_logo_url} alt={siteFolder.name} className={siteFolder.photo_url ? "w-full h-full object-cover" : "max-h-full max-w-full object-contain p-1"} onError={(e) => { e.target.style.display = 'none'; }}/>) : (<MapPin className="h-7 w-7 text-muted-foreground"/>)}
                  </div>} title={siteFolder.name} stats={[
                    ...
                    <CardActions id={siteFolder.id} type="site" name={siteFolder.name} onEdit={() => setEditingSite({ id: siteFolder.id, name: siteFolder.name, city: '', state: '' })}/>
                    <DownloadButton reportIds={siteFolder.reports.map(r => r.id)} folderName={`${selectedCompany.name}_${siteFolder.name}`} folderId={`site-${siteFolder.id}`} size="sm"/>
                  </>}/>))}
          </div>
        </div>
        {dialogs}
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between bg-card p-4 rounded-xl border shadow-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            <Select value={String(selectedMainYear)} onValueChange={(val) => setOpenYear(Number(val))}>
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 gap-6">
          {filteredCompanyFolders.map((company) => (<FolderCard key={company.id} onClick={() => {
            ...
            }} badge={<Badge variant="secondary" className="text-[10px] px-2 py-0.5 bg-neutral-700 text-neutral-300 border-0">
                  Fábrica
                </Badge>} icon={<div className="w-20 h-[4.2rem] rounded-lg overflow-hidden flex items-center justify-center p-1" style={{ backgroundColor: '#ffffff' }}>
                  {(company.logo_url || company.photo_url) ? (<img src={company.logo_url || company.photo_url} alt={company.name} className="max-h-full max-w-full object-contain" onError={(e) => { e.target.style.display = 'none'; }}/>) : (<Building2 className="h-10 w-10 text-muted-foreground"/>)}
                </div>} title={company.name} stats={company.totalCount === 0
                ...
                : [
                  ...
                  <CardActions id={company.id} type="company" name={company.name} onEdit={() => navigate(`/super-admin?tab=companies&edit=${company.id}`)}/>
                  <DownloadButton reportIds={company.reports.map((r) => r.id)} folderName={company.name} folderId={`company-${company.id}`} size="sm"/>
                </>}/>))}
        </div>
      </div>
