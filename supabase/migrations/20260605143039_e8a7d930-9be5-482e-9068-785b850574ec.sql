...
BEGIN
  ...
  LOOP
    ...
    SELECT count(*) INTO v_match_count FROM _profiles_norm pn WHERE pn.norm = v_norm;

    IF v_match_count = 1 THEN
      SELECT pn.id, pn.original_name INTO v_match_id, v_match_name
        FROM _profiles_norm pn WHERE pn.norm = v_norm LIMIT 1;
      v_nivel := 'exato';
    ...
    ELSE
      ...
      IF v_first <> '' AND v_last <> '' AND v_first <> v_last THEN
      ...
      IF v_nivel IS NULL THEN
        SELECT count(*) INTO v_match_count
          FROM _profiles_norm pn WHERE similarity(pn.norm, v_norm) >= 0.85;

        IF v_match_count = 1 THEN
          SELECT pn.id, pn.original_name INTO v_match_id, v_match_name
            FROM _profiles_norm pn WHERE similarity(pn.norm, v_norm) >= 0.85 LIMIT 1;
