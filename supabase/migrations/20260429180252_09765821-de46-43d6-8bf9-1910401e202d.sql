CREATE OR REPLACE FUNCTION public.resolve_client_portal_branding(p_company_id uuid DEFAULT NULL::uuid, p_site_id uuid DEFAULT NULL::uuid)
...
BEGIN
  ...
  -- 1) Resolve site for context preservation only (not used for logo)
  ...
  -- Fallback: client/contact company
  ...
  -- LOGO: sempre da EMPRESA (fábrica). Nunca usar foto da unidade.
  -- Prioridade: companies.logo_url -> companies.photo_url
  RETURN QUERY
  SELECT
    c.id AS company_id,
    v_site_id AS site_id,
    c.name AS name,
    COALESCE(c.logo_url, c.photo_url) AS logo_url
